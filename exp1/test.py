...
def train_and_test(images_train, labels_train, images_test, labels_test, images_validation, labels_validation):
    ...
    # 输出层
    W3 = tf.Variable(tf.random_normal([H2_NN, 10]))
        ...
        for (batchImages, batchLabels) in batch_iter(images_train, labels_train, batch_size, epoch_num, shuffle=True):
            ...
            if acc >= 0.97:
                saver.save(sess, os.path.join(
                    modelFilePath, 'mnist_h256_model.ckpt'
                ))
                break
        accu_test = sess.run(accuracy, feed_dict={
            x: images_validation, y: labels_validation
...
if __name__ == '__main__':
    ...
    # 1、使用简单划分的训练集和测试集训练，并使用测试集评估模型
    train_and_test(origin_images_train, origin_labels_train, origin_images_test,
                   origin_labels_test, origin_images_test, origin_labels_test)
    # 2、调用函数用留出法和k折交叉验证法评估模型
